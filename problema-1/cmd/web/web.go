...
import (
	...
	"github.com/do-prueba-tecnica/problema-1/internal/application"
...
Options:
	...
	assertor.IntBetween(port, 1024, 65535, "port in web adapter out of range")
	assertor.IntNot(port, 22, "port cannot be 22")
	assertor.PortClosed(port, "the port is closed")

	app := application.NewApp(stderr, stdout, format)
